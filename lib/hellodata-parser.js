...
export function parseHelloData(buffer) {
  ...
  // subject is the FIRST non-metadata column after the "Property Name" header
  let subjectCi = -1
  if (propNameCi >= 0 && compsAvgCi >= 0) {
    for (let i = propNameCi + 1; i < compsAvgCi; i++) {
      const s = String(hdr[i] || '').trim()
      if (s && !/property name|management|website/i.test(s)) { subjectCi = i; break }
    }
  }
  const propCols = []
  hdr.forEach((v, ci) => {
    ...
    propCols.push({ ci, name: s, isSubject: ci === subjectCi })
  })

...
const FEE_CODES = new Set([
  ...
  // Pine25 / Yardi variants
  'garagmsc','vtrash','amenity','pest','resins','rubs','pkg','storage',
  'concession','conc','leaseup','specl','disc'
])
...
function parseRows(rows) {
  ...
  // Auto-detect rent code: find the non-fee positive charge with the highest median amount.
  // Using median (not frequency) prevents small recurring fees ($10 parking, etc.) from
  // being mistaken for rent when they appear on more units than the actual rent code.
  ...
  for (const row of rows) {
    ...
    if (code && code !== 'total' && !FEE_CODES.has(code) && amt != null && amt > 0) {
      ...
      codeAmounts[code].push(amt)
    }
  }
  // Only auto-detect a rent code if none of the standard known codes appear in the file.
  // This prevents common small fees (amenity $10, vtrash $25) from being mis-identified
  // as rent when the file already uses a standard code like 'rent'.
  const knownRentFound = Object.keys(codeAmounts).some(c => KNOWN_RENT.has(c))
  if (!knownRentFound) {
    const median = arr => { const s = [...arr].sort((a,b)=>a-b); const m = Math.floor(s.length/2); return s.length%2 ? s[m] : (s[m-1]+s[m])/2 }
    const topUnknown = Object.entries(codeAmounts)
      .filter(([c, amts]) => !KNOWN_RENT.has(c) && !FEE_CODES.has(c) && amts.length >= 3 && median(amts) >= 500)
      .sort((a, b) => median(b[1]) - median(a[1]))[0]
    if (topUnknown) KNOWN_RENT.add(topUnknown[0])
  }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (looksUnit) {
      ...
      units.push(cur)
      // Some exports (e.g. Yardi Pine25) put the first charge on the unit header row
      const hcode = G == null ? '' : String(G).trim().toLowerCase()
      const hamt = num(H)
      if (hcode === 'total' && hamt != null) cur.total = hamt
      else if (KNOWN_RENT.has(hcode) && hamt != null) cur.rentlines.push(hamt)
      else if (FEE_CODES.has(hcode) && hamt != null) cur.fees += hamt
    } else if (cur) {

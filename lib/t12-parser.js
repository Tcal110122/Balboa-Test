...
function parseT12Data(grid) {
  ...
  const potentialRent = fig('41029-099', 'Potential Rent',         /^(potential( gross)? rent|gross potential( rent)?|gross possible( rent)?|scheduled rent|total possible rent|total net rent)\s*$/i)
  ...
  const expenseCats = [
    ...
    ['54999-099', 'Advertising / Marketing',  /^(total )?(advertising(\s*(&|and)\s*promotion)?|marketing)(\s+expenses?)?\s*$/i],
    ...
    ['62999-099', 'Taxes',                    /^(total )?(tax(es?)?(\s*[&and]+\s*insurance)?(\s+expense)?|real estate tax(es?)?)\s*$/i],
    ['63999-099', 'Insurance',                /^(total )?insurance(\s+expense)?\s*$/i]
  ]

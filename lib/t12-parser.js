...
function parseT12Data(grid) {
  ...
  const findRow = (re) => {
    for (let r = hdrRow + 1; r < grid.length; r++) {
      for (let ci = 0; ci < 6; ci++) {
        if (re.test(String((grid[r] || [])[ci] || '').trim())) {
          const d = readRow(r)
  ...
  const totalIncome  = fig('49999-999', 'Total Income',            /^(total income|total revenue|effective gross income|egi|income)\s*$/i)
  const totalOpEx    = fig('66999-199', 'Total Operating Expenses', /^(total (oper|expens)|operating expenses)\s*$/i)
  const noi          = fig('69999-099', 'Net Operating Income',    /^net operating income\s*$/i)
  ...
  const expenseCats = [
    ['51599-099', 'Payroll & Benefits',       /^(total )?payroll(\s*&\s*benefits)?\s*$/i],
    ['53999-099', 'General Maintenance',      /^(total )?(general maintenance|repairs?\s*&\s*maint(enance)?|contract services)\s*$/i],
    ['54999-099', 'Advertising / Marketing',  /^(total )?(advertising|marketing)(\s+expenses?)?\s*$/i],
    ['58399-099', 'General & Administrative', /^(total )?general(\s*&\s*|\s+)admin(istrative)?\s*$/i],
    ['59999-099', 'Utilities',                /^(total )?utilities\s*$/i],
    ['61999-099', 'Management Fees',          /^(total )?management fees?\s*$/i],
    ['62999-099', 'Taxes',                    /^(total )?(taxes?\s*(&\s*insurance)?|real estate taxes?)\s*$/i],
    ['63999-099', 'Insurance',                /^(total )?insurance\s*$/i]
  ]
  ...
  const incomeLines = [
    fig('41029-099', 'Potential Rent',          /^(potential (gross )?rent|scheduled rent|gross potential)\s*$/i),
    fig('41999-098', 'Total Other Rental Inc.', /^(total )?other rental (income|inc\.?)\s*$/i),
    fig('43599-099', 'Total Other Income',      /^(total )?other income\s*$/i),
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (/\w+ \d{4}\s*[-–]\s*\w+ \d{4}/.test(v) || /period\s*=/i.test(v)) {
      period = v.replace(/period\s*=\s*/i, '').trim(); break
    }
  }

...
function parseT12Data(grid) {
  ...
  const expenseCats = [
    ...
    ['54999-099', 'Advertising / Marketing',  /^(total\s+)?(advertising(\s*([\/&]|and)\s*(promotion|marketing))?\s+expenses?|marketing\s+expenses?|total\s+(advertising(\s*([\/&]|and)\s*(promotion|marketing))?|marketing))(\s+expenses?)?\s*$/i],
  ...
  const otherRentalIncRe = [
    ...
    /^vacancies\s*(&|and)\s*concessions?\s*$/i,  // Ascend format subtotal only
  ]
  ...
  // Prefer actual "Other Income" subtotal (blank col 0); only derive when it doesn't exist
  let otherIncFig = figSubtotal('43599-099', 'Total Other Income', /^(total )?other income\s*$/i)

...
function mmddyyyyToLabel(v) {
  if (v instanceof Date && !isNaN(v)) return MON_NAMES[v.getUTCMonth()] + ' ' + v.getUTCFullYear()
  if (typeof v === 'number' && v > 40000 && v < 80000) {
    const d = new Date(Math.round((v - 25569) * 86400 * 1000))
    if (!isNaN(d)) return MON_NAMES[d.getUTCMonth()] + ' ' + d.getUTCFullYear()
  }
...
function parseT12Data(grid) {
  const isDateLike = v => (v instanceof Date && !isNaN(v)) || (typeof v === 'number' && v > 40000 && v < 80000)
  ...
  for (let i = 0; i < 14 && i < grid.length; i++) {
    ...
    const cnt = row.filter(v => {
      ...
      return /^[A-Za-z]{3,}\s+\d{4}$/.test(s) || /^\d{1,2}\/\d{2}\/\d{4}$/.test(s) || isDateLike(v)
    }).length
  ...
  ;(grid[hdrRow] || []).forEach((v, ci) => {
    ...
    if (/^[A-Za-z]{3,}\s+\d{4}$/.test(s)) months.push({ ci, label: s })
      ...
      const lbl = mmddyyyyToLabel(v)  // handles both Date objects and numeric serials
      if (lbl) months.push({ ci, label: lbl })
  ...
  // findSubtotalRow: like findRow but ONLY matches rows where col 0 is blank
  // (subtotal rows in this format have no account code in col 0).
  // Falls back to regular findRow if no subtotal match found.
  const findSubtotalRow = (re) => {
    for (let r = hdrRow + 1; r < grid.length; r++) {
      const col0 = String((grid[r] || [])[0] || '').trim()
      if (col0) continue // skip rows with account codes
  ...
  // figSubtotal: like fig but uses findSubtotalRow to prefer blank-col-0 rows
  const figSubtotal = (acct, label, re) => {
    let r = byAcct[acct]
    if (r == null && re) r = findSubtotalRow(re)
    if (r == null) return null
    const d = readRow(r)
    return { label, acct, row: r, vals: d.vals, total: d.total }
  }
  ...
  const totalIncome  = figSubtotal('49999-999', 'Total Income',    /^(total (income|revenue)|effective gross income|egi|total revenue|income)\s*$/i)
  const totalOpEx    = fig('66999-199', 'Total Operating Expenses', /^(total (operating )?expens(es?)?|operating expenses?|total expenses?)\s*$/i)
  const noi          = figSubtotal('69999-099', 'Net Operating Income', /^(net operating income\/?(\(loss\))?|net income|noi)\s*$/i)
  ...
  const potentialRent =
    ...
    fig('41029-099', 'Potential Rent', /^(potential( gross)? rent|gross potential( rent)?|gross possible( rent)?|scheduled rent|total possible rent|total net rent|net potential rent)\s*$/i)

  const maintRe = [
    /^(total )?(general maintenance|repairs?\s*&\s*maint(enance)?|maintenance(\s*&?\s*repair)?|property\s+repairs?\s*&\s*maint(enance)?)\s*$/i,
    /^(total )?(painting(\s*(&|and)\s*decorat(ing)?)?|redecorat\w*(\s*[\/&]?\s*make.?ready)?|make.?ready)\s*$/i,
    /^(total )?(contracted?\s*services?|contract\s*services?|landscape\s*(and|&)?\s*contract\s*services?)\s*$/i,
    /^(total )?(landscape(\s*(maintenance|contract|services?|and\s+contract\s*services?))?)\s*$/i,
  ]
  const expenseCats = [
    ['51599-099', 'Payroll & Benefits',       /^(total )?(payroll(\s*&\s*benefits|\s+expense)?|salaries(\s+expense)?)\s*$/i],
    ['54999-099', 'Advertising / Marketing',  /^(total\s+)?(advertising(\s*([\/&]|and)\s*(promot\w*|marketing))?\s+expenses?|marketing\s+expenses?|total\s+(advertising(\s*([\/&]|and)\s*(promot\w*|marketing))?|marketing))(\s+expenses?)?\s*$/i],
    ...
    ['62999-099', 'Taxes',                    /^(total )?(real\s+(estate|property)\s+tax(es?)?|property\s+tax(es?)?|tax(es?)?(\s+expense)?|fixed\s+expenses?)\s*$/i],
  ]
  const insurRe = [
    /^(total )?insurance(\s+expense)?\s*$/i,
    /^(property\s*(&|and)\s*liability\s*)?insurance\s*$/i,
    /^flood\s*insurance\s*$/i,
    /^property\s+insurance\s*$/i,
  ]
  ...
  // Other Rental Inc: prefer an explicit subtotal row; only fall back to summing "Less:" rows
  // if no subtotal exists.  This avoids double-counting when both are present.
  const otherRentalIncSubtotalRe = /^total\s+(other\s+rental\s+(income|inc\.?)|rental\s+(losses?|income|adjustments?))\s*$/i
  let otherRentalIncFig = fig('41999-098', 'Total Other Rental Inc.', otherRentalIncSubtotalRe)
  if (!otherRentalIncFig) {
    const otherRentalIncRe = [
      /^less:?\s*concessions?/i,
      /^less:?\s*vacancy(\s+loss(es?)?)?/i,
      /^less:?\s*write.?offs?/i,
      /^less:?\s*employee(\s+rent)?\s+concession/i,
      /^(total )?(other rental (income|inc\.?)|rental losses?)\s*$/i,
      /^vacancies\s*(&|and)\s*concessions?\s*$/i,
    ]
    otherRentalIncFig = figMany('41999-098', 'Total Other Rental Inc.', otherRentalIncRe)
  }

  // Other Income: prefer an explicit "TOTAL OTHER INCOME" subtotal
  let otherIncFig = figSubtotal('43599-099', 'Total Other Income', /^total\s+other\s+income\s*$/i)
  if (!otherIncFig) otherIncFig = figSubtotal('43599-099', 'Total Other Income', /^(total )?other income\s*$/i)
  if (!otherIncFig && totalIncome && potentialRent && otherRentalIncFig) {

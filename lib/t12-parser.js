...
function parseT12Data(grid) {
  ...
  if (!noiAfterRepl && noi && belowNOI.length) {
    const replVals = noi.vals.map((v, i) => {
      const rv = belowNOI.reduce((s, c) => s + (c.vals && c.vals[i] != null ? c.vals[i] : 0), 0)
      return v != null ? v - rv : null
    })
    const replTotal = belowNOI.reduce((s, c) => s + (c.total || 0), 0)
    noiAfterRepl = { label: 'NOI After Replacements', acct: 'computed', row: -1,
      vals: replVals, total: (noi.total || 0) - replTotal }
  }

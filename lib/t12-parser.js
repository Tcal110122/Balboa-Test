...
function parseT12Data(grid) {
  ...
  const potentialRent = fig('41029-099', 'Potential Rent',         /^(potential( gross)? rent|gross potential( rent)?|gross possible( rent)?|scheduled rent)\s*$/i)

  const expenseCats = [
    ['51599-099', 'Payroll & Benefits',       /^(total )?payroll(\s*&\s*benefits|\s+expense)?\s*$/i],
    ['53999-099', 'General Maintenance',      /^(total )?(general maintenance|repairs?\s*&\s*maint(enance)?|contract services|maintenance\s*&?\s*repair)\s*$/i],
